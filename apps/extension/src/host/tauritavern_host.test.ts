    ...
    api: {
      ...
      worldInfo: {
        getLastActivation: async () =>
          (options?.last_activation ?? null) as TauriWorldInfoActivationBatchSurface | null,
...
describe("TauriTavernHost", () => {
  it("registers a protocol-v1 ChatSurface participant and normalizes hook disposers", () => {
    ...
    if (typeof mounted_disposer !== "function" || typeof commit_disposer !== "function") {
  ...
  it("detects Tauri only through window.__TAURITAVERN__ and waits for readiness", async () => {
    ...
    await expect(create_tauritavern_host(globals)).resolves.toBeInstanceOf(TauriTavernHost);
    expect(ready).toBe(true);
    await expect(create_tauritavern_host({ __TAURITAVERN__: undefined })).resolves.toBeUndefined();
  });
});
...
it("accepts a property-readable ChatSurface registration with a prototype trap", () => {
  const fault = vi.fn();
  const registration_result = new Proxy(
    { fault },
    {
      getPrototypeOf: () => {
...
it.each([{}, { dispose: "invalid" }])(
  "rejects a malformed ChatSurface hook disposer",
  (raw_disposer) => {
    const tauri = create_tauri_host();
    tauri.host.register_chat_surface({
      id: "tavern-canvas.messages",
...
class BrandedActivationBatch {
  readonly timestampMs = 1;
  readonly trigger = "generation";
  readonly entries: readonly unknown[] = [];
}

class BrandedActivationEntry {
  readonly world = "Lore";
  readonly uid = 1;
  readonly displayName = "Entry";
  readonly constant = true;
}

it.each([
  ["a scalar batch", 7],
  ["an invalid timestamp", { timestampMs: Number.NaN, trigger: "generation", entries: [] }],
  ["an invalid trigger", { timestampMs: 1, trigger: null, entries: [] }],
  ["non-array entries", { timestampMs: 1, trigger: "generation", entries: null }],
  ["a non-record entry", { timestampMs: 1, trigger: "generation", entries: [null] }],
  [
    "an invalid world",
    {
      timestampMs: 1,
  ...
  ["a custom-prototype activation batch", new BrandedActivationBatch()],
  [
    "a custom-prototype activation entry",
    {
      timestampMs: 1,
      trigger: "generation",
      entries: [new BrandedActivationEntry()],
    },
  ],
])("rejects %s", async (_, last_activation) => {
  const tauri = create_tauri_host({ last_activation });

  await expect(tauri.host.get_last_world_info_activation()).rejects.toThrowError(
    /invalid WorldInfo activation/u,
  );
});
it.each([
  ["Date", new Date()],
...
it.each([undefined, null, {}, "invalid"])(
  ...
  async (unsubscribe_result) => {
    ...
    await expect(tauri.host.subscribe_world_info_activation(() => undefined)).rejects.toThrowError(
      /invalid WorldInfo unsubscribe/u,
    );
  },
);
...
it("keeps async WorldInfo disposal idempotent after an error", async () => {
  ...
  const dispose = await tauri.host.subscribe_world_info_activation(() => undefined);
...
describe("inspect_tauritavern", () => {
  ...
  it.each(["protocolVersion", "isManagedOwnershipRequired", "registerParticipant"])(
    "requires ChatSurface %s",
    (property_name) => {
  ...
  it.each(["abiVersion", "api"])("fails closed when the %s getter throws", (property_name) => {
